...
namespace PrimePenguin.CentraSharp.Services.Customer
{
    ...
    public class CustomerService : CentraService
    {
        ...
        public virtual async Task<CentraCustomerList> ListAsync(CustomerFilter options = null)
        {
            var req = PrepareRequest("customers");

            if (options != null) req.QueryParams.AddRange(options.ToParameters());

            return await ExecuteRequestAsync<CentraCustomerList>(req, HttpMethod.Get);
        }
        ...
        public virtual async Task<CentraCustomerList> GetAsync(long customerId)
        {
            var req = PrepareRequest($"customers/{customerId}");
            return await ExecuteRequestAsync<CentraCustomerList>(req, HttpMethod.Get);
        }
        /// <summary>
        /// Update cusromer for given customerID
        /// </summary>
        /// <param name="updateCustomer"></param>
        /// <param name="cutomerId"></param>
        /// <returns></returns>
        public virtual async Task<CentraCustomerList> UpdateAsync(UpdateCustomerOptions updateCustomer, int cutomerId)
        {
            ...
            return await ExecuteRequestAsync<CentraCustomerList>(req, HttpMethod.Put, content);
        }
    }
}